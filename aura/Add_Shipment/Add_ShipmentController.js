...
AddSB : function(component, event, helper) {
    ...
    // READ VALUE FROM EVENT (NOT ATTRIBUTE)
    let insuranceVal = event.getSource().get("v.value");

    console.log('Insurance_By__c (event) : ', insuranceVal);

    // normalize (PROD SAFE)
    insuranceVal = insuranceVal ? insuranceVal.trim().toUpperCase() : '';

    // sync attribute manually
    component.set("v.InsuranceBy1", insuranceVal);

    // show / hide Liability field
    component.set("v.showLiability_Type", insuranceVal === 'BVC');

    // set default Liability value
    component.set(
        "v.Liability_Type",
        insuranceVal === 'BVC' ? 'All Risk Liability' : 'None'
    );
},
       ...
       // action.setParams({recId:x});
        action.setCallback(this,function(resp){
            ...
            {                
                ...
                console.log('741 r.Customer__r.New_Contract__c :'+r.Customer__r.New_Contract__c);
            } else if (action.getState() === 'ERROR'){
                var errors = action.getError();
                if (errors) {
                    if (errors[0] && errors[0].message) {
