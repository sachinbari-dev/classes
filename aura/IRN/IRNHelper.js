...
/*
    ...
    UpdateShip3 : function(component,event,helper) {
       ...
       component.set("v.sac",true);
        
        component.set("v.sad",true);
        ...
        console.log('InsuranceBy1 :'+InsuranceBy1);
          ...
          var stopSaving=false;
        if( InsuranceBy1=='BVC' && Liability_Type1=='')
        {
            stopSaving=true;
            var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
        ...
        stopSaving=true;
            var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    "title": "error!",
                    "type" :"error",
                    "message": 'Net weight is greater than gross weight.'
                });
                toastEvent.fire();
            ...
            if( stopSaving !=true && InsuranceBy1!='None' && customerGW>0)
        {
...
UpdateShip1 : function(component, event, helper) {
    ...
    var InsuranceBy1    = component.get("v.InsuranceBy1");
    var Liability_Type1 = component.get("v.Liability_Type");
    var customerGW      = component.get("v.CustomerGW");
    var nwcTotal        = component.get("v.nwc_Total");
    var AdditionalRemarks = component.get("v.AdditionalRemarks");
    ...
    if (!InsuranceBy1 || InsuranceBy1 === 'None') {
        helper.showToast('Error!', 'Insurance By Value Cannot Be None.', 'error');
        return;
    }
    ...
    if (
        InsuranceBy1 == 'BVC' &&
        (Liability_Type1 == 'None' || Liability_Type1 =='')
    ) {
        helper.showToast('Error!', 'Fill Liability_Type', 'error');
        return;
    }

    
    if (customerGW && customerGW > 0 && nwcTotal > customerGW) {
        helper.showToast('Error!', 'Net weight is greater than gross weight.', 'error');
        return;
    }
    ...
    action.setCallback(this, function(resp) {
        ...
        if (resp.getState() === 'SUCCESS' && resp.getReturnValue()) {
            ...
            helper.showToast('Success!', 'Shipment Saved successfully.', 'success');

            var r = resp.getReturnValue();
            component.set("v.Shipment", r);
            component.set("v.editPage", true);
            component.set("v.hide2", false);
    ...
    // Added By sanket For Customer  gross weight Update.
    UpdateShip2 : function(component, event, helper) {

    var customerGrossWeight = component.get("v.CustomerGW");
    var shipId = component.get("v.Shipment.Id");

    console.log('Updating Customer Gross Weight:', customerGrossWeight);
        ...
        if (state === 'SUCCESS') {
            ...
            if (result) {
                ...
                toastEvent.setParams({
                    title: "Success!",
                    type: "success",
                    message: "Customer Gross Weight updated successfully."
                });
                toastEvent.fire();
            }
    ...
    saveInvoice12 : function(component,event,helper){
        ...
        action.setCallback(this,function(resp){
            console.log("1");
            //alert('helper alert action.getState() == '+action.getState());
            if (action.getState() === 'SUCCESS')
            {          
                console.log('Inside CD Success');
                console.log('Res :'+resp.getReturnValue());
                //alert('helper alert resp.getReturnValue() == '+resp.getReturnValue());
                if(resp.getReturnValue() != null)
                {
...
//Added Code By Rafi Khan for Escan shipment auto creation
    saveEScanShipment : function(component,event,helper) {
        var nwc = component.get("v.nwc");
        var refNo = component.get("v.refNo");
        var IValue = component.get("v.IValue");
        ...
        var D_Address = component.get("v.D_Address");
        */
        //var CS_ACCID = component.get("v.qrCodeData.Id");
        
        var CS_ACCID = component.get("v.qrCodeData.Id") || component.get("v.SelHub.val");
        var D_Address = component.get("v.SelHub1.val");
        ...
        /*if(CS_ACCID == null){ window.alert('Select Consignee Account');
            component.set("v.sac",false);}else{component.set("v.sac",true);}*/
        if(D_Address == null){
            window.alert('Select Destination Address');
            component.set("v.sad",false);
        }else{
            component.set("v.sad",true);
        }
        ...
        */
        ...
        console.log('13. customerGrossWeight :'+customerGrossWeight); 
        console.log('InsuranceBy1 :'+InsuranceBy1);
        //if(CS_ACCID != null && D_Address != null && InsuranceBy1!='None' && ((BVCServices1!='None' && ProdcutDescription1!='None') || 
          //                                                                                            ((BVCServices1 == 'eSHIP' ||BVCServices1 == 'BATH' || BVCServices1 == 'LuxeSHIP') && ProdcutDescription1=='None'))){
            if(stopSaving!=true && CS_ACCID != null && D_Address != null && InsuranceBy1!='None' && customerGrossWeight>=0 && ((BVCServices1!='None' && ProdcutDescription1!='None') || 
           ((BVCServices1 == 'eSHIP' ||BVCServices1 == 'BATH' || BVCServices1 == 'LuxeSHIP') && ProdcutDescription1=='None'))){
        ...
        updateProductDescriptions: function(component, ServiceCode, ProdDesc) {
            //alert('In helper ServiceCode :'+ServiceCode);
            //alert('In helper ProdDesc :'+ProdDesc);
            var productDescriptions;
            
            if (ServiceCode === 'ValSHIP') {
                productDescriptions = [
                    { id: 'None', label: 'None',selected: true },
                    { id: 'Cut & Polished Diamonds', label: 'Cut & Polished Diamonds' },
                    { id: 'DIAMOND JEWELLERY', label: 'DIAMOND JEWELLERY' },
            ...
            } else if (ServiceCode === 'SilverSHIP') {
                productDescriptions = [
                    { id: 'None', label: 'None',selected: true },
                    { id: 'Coloured & Gem Stones', label: 'Coloured & Gem Stones' },            
                    { id: 'Imitation Jewellery', label: 'Imitation Jewellery' },
            ...
            component.set("v.ProductDescriptions", productDescriptions);
            //alert('productDescriptions '+productDescriptions);
            //alert('JSON.stringify(productDescriptions) '+JSON.stringify(productDescriptions));
            //alert('productDescriptions.length :'+productDescriptions.length);
            //alert('ProdDesc ==='+ProdDesc+'...');
            
            //alert('[' + ProdDesc + '] length=' + ProdDesc.length);

            var cleanProdDesc = (ProdDesc || '').trim();
            //alert(' cleanProdDesc ==='+ cleanProdDesc +'...');

            for (var i = 0; i < productDescriptions.length; i++) {
                var item = productDescriptions[i].id || '';
                //alert('in for item ==='+ item +'...');
                //if (productDescriptions[i].id === ProdDesc) {
                if (item.indexOf(cleanProdDesc) !== -1) {
                    //alert('Inside if item '+item);
                    component.set("v.c_pdes", item);
                    //alert('component.get(v.c_pdes) '+component.get("v.c_pdes"));
                    break;
                }
            }
            ...
            isValidProductDescription: function(serviceCode, prodDesc) {
                const map = {
                    "ValSHIP": [
                        ...
                        "Multiple Fine Jewellery Products"
                    ],
                    "SilverSHIP": [
                        "Coloured & Gem Stones",
                        "Imitation Jewellery",
                ...
                if(!serviceCode || !prodDesc) return false;
                
                return map[serviceCode] && map[serviceCode].includes(prodDesc);
            }
