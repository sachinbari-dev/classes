({
    saveShipment : function(component,event,helper) {
        ...
        */
        ...
        var ConsigneeSlection = component.get("v.isScannedByQRCode");
        if(ConsigneeSlection){
...
/*
		...
		var action = component.get('c.SaveShipmentforInt');
         action.setParams({
             nwc : nwc,
             refNo : refNo,
             IValue : IValue,
    ...
    UpdateShip1 : function(component,event,helper) {
       ...
       // var nwc = component.get("v.nwc");
        ...
        if(CS_ACCID != null && D_Address != null && InsuranceBy1!='None' && customerGrossWeight>0 && ((BVCServices1!='None' && ProdcutDescription1!='None') || 
           ((BVCServices1 == 'eSHIP' ||BVCServices1 == 'BATH' || BVCServices1 == 'LuxeSHIP') && ProdcutDescription1=='None'))){
            component.set("v.spinner",'true');
    ...
    setOptions : function(component,event,helper) {
        var BVCServices =  [
            ...
            { id: 'ValSHIP', label: 'ValSHIP', value:'ValSHIP' , disabled: 'false' },
            { id: 'GoldSHIP', label: 'GoldSHIP', value:'GoldSHIP', disabled: 'false' },
            { id: 'SilverSHIP', label: 'SilverSHIP', value: 'SilverSHIP' , disabled: 'false' },
    ...
    compressedImage: function(component, dataUrl, helper, invoiceId, lat, long, username) {
        ...
        img.onload = () => {
            ...
            ctx.fillText(`Username: ${username}`, 10, canvas.height - 60);
            ctx.fillText(`Datetime: ${new Date().toLocaleString()}`, 10, canvas.height - 45);
            ctx.fillText(`Latitude: ${lat}`, 10, canvas.height - 30);
            ctx.fillText(`Longitude: ${long}`, 10, canvas.height - 15);
    ...
    //Added By Rafi Khan for auto population of product description depending on BVC Service after scanning QR
    updateProductDescriptions: function(component, ServiceCode, ProdDesc) {
            ...
            for (var i = 0; i < productDescriptions.length; i++) {
                var cleanLabel = productDescriptions[i].label.replace(/\u2060/g, '').trim();
                var cleanProdDesc = ProdDesc.replace(/\u2060/g, '').trim();
                
                if (cleanLabel===cleanProdDesc) {
                    //alert('productDescriptions[i].label '+productDescriptions[i].id);
                    component.set("v.c_pdes", productDescriptions[i].label);
